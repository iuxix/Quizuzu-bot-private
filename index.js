...
async function fetchQuiz() {
  const { data } = await axios.get("https://opentdb.com/api.php?amount=1&category=19&type=multiple"); // Math only
...
async function sendQuiz(chatId, user, isGroup = false) {
  ...
  bot.sendPoll(chatId, quiz.question, quiz.answers, {
    ...
    is_anonymous: false,
    explanation: "🎯 Be quick and correct! Use /hint if stuck."
  });
}
...
bot.onText(/^\/start$/, async (msg) => {
  await getUser(msg);
  const menu = `
🤖 *Welcome to Deb’s Quiz Bot!*

🎮 *Commands:*
/quiz – 🧠 Start a quiz
/fight – ⚔️ Start group quiz
/points – 💰 See points & badges
/profile – 👤 View/edit profile
/ranks – 🏅 View rank levels
/stats – 📊 Full progress
/leaderboard – 🏆 Group top 10
/hint – 💡 Get hint (3/day)
/answer – ℹ️ See correct answer

🌟 /setnick <name> – Custom nickname
🌟 /setavatar <emoji> – Your emoji avatar
...
bot.onText(/^\/fight$/, async msg => {
  if (!msg.chat.type.endsWith("group")) {
    return bot.sendMessage(msg.chat.id, "⚠️ *Fight only available in group chats!*", { parse_mode: "Markdown" });
  }
  const user = await getUser(msg);
...
bot.on('poll_answer', async (answer) => {
  ...
  const key = keys.find(k => k.endsWith(`:${user.id}`));
  if (!key) return;
  ...
  if (answer.option_ids.includes(last.correctIndex)) {
    const fast = (now - last.time < 30000);
    const bonus = fast ? 2 : 1;
    user.points += bonus;
    user.streak++;
...
bot.onText(/^\/profile$/, async (msg) => {
  ...
  const txt = [
    ...
    `💰 *Points*: ${user.points}`,
    `🔥 *Streak*: ${user.streak}`,
    `🎖 *Badges*: ${user.badges.length ? user.badges.join(', ') : 'None'}`,
    "",
...
bot.onText(/^\/leaderboard$/, async (msg) => {
  if (!msg.chat.type.endsWith("group"))
    return bot.sendMessage(msg.chat.id, "🏆 Group-only command. Add me to a group!", { parse_mode: "Markdown" });
...
// === /broadcast (media + text) for admin only ===

bot.onText(/^\/broadcast$/, (msg) => {
  if (String(msg.from.id) !== String(ADMIN_ID)) return;
  bot.sendMessage(msg.chat.id, "📨 Send the message or media to broadcast to all users.").then(() => {
    broadcastWaiters.add(msg.from.id);
  });
});

bot.on('message', async (msg) => {
  if (!broadcastWaiters.has(msg.from.id)) return;
  if (String(msg.from.id) !== String(ADMIN_ID)) return;
